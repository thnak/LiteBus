    ...
    public void Register([DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] Type type)
    {
        ...
        lock (_lock)
        {
            ...
            if (newDescriptors.Count == 0)
            {
                ...
                // Skip open generic type definitions — they are not concrete message types.
                if (!type.IsGenericTypeDefinition)
                    RegisterMessageType(type);
            }
            else if (type.IsGenericTypeDefinition && newDescriptors.Any(d => d.MessageType.IsGenericParameter))
            {
                // Open generic handler with a bare type parameter (e.g. Foo<T> : ICommandPreHandler<T>).
                // These must be registered as concrete closed forms via the source generator
                // (e.g. typeof(Foo<ConcreteCommand>)).  Silently skip — no runtime MakeGenericType.
            }
            else
            {
                // Concrete or generic-message handler — process normally.
                // Generic handlers typed to a specific generic message (e.g. Handler<T> : IHandler<Msg<T>>)
                // are processed here; GetHandlerType closes them at dispatch time.
